...
class VariationalCredalConfig:
    ...
    # Legacy MAQA-style uncertainty calibration fields.
    # These are kept for checkpoint compatibility with older snapshots.
    ...
    lambda_ale_mse: float = 1.0
    lambda_ale_rank: float = 2.0
    lambda_epi_mse: float = 1.0
    lambda_epi_rank: float = 1.0
    lambda_decorr: float = 20.0
    lambda_variance: float = 0.5
    rank_margin: float = 0.1
    decorr_target: float = 0.0

    def __post_init__(self) -> None:
        if isinstance(self.covariance_family, str):
            self.covariance_family = CovarianceFamily(self.covariance_family)

    @classmethod
    def from_dict(cls, data: Dict) -> "VariationalCredalConfig":
        """Build a config while ignoring unknown legacy keys."""
...
class VariationalCredalCBM(nn.Module):
    ...
    def forward(
        ...
        result = {
            ...
            'label_logits': task_out['logits'],
            'label_probs': task_out['probs'],

            # Concept outputs
            'concept_probs': concept_probs,
            'mu': concept_probs,
            'concept_logits': concept_out['logits_mean'],
            ...
            'epistemic': epistemic,
            'aleatoric': aleatoric,
            'sigma_epi': epistemic.mean(dim=-1) if epistemic.dim() > 1 else epistemic,
            'sigma_ale': aleatoric.mean(dim=-1) if aleatoric.dim() > 1 else aleatoric,
    ...
    def _compute_losses(
        ...
        """Compute all losses with structural separation"""
        losses = {}

        # Task loss
        if labels is not None:
            losses['ce_loss'] = F.cross_entropy(result['logits'], labels)

        # KL loss (epistemic regularization)
