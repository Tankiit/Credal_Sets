...
Each backbone exposes a common interface:

    backbone = build_backbone(kind="hf", model_name="facebook/dinov2-base", device="cuda")
    features = backbone.encode_pil(images)

The contract is intentionally small so downstream uncertainty analysis only consumes
an ``(N, D)`` embedding matrix and does not depend on model-specific preprocessing.
"""
...
class HuggingFaceBackbone(FrozenVisualBackbone):
    ...
    Pooling policy:
      1. ``pooler_output`` when provided by the model;
      2. otherwise CLS token ``last_hidden_state[:, 0]``;
      3. otherwise mean-pool spatial/sequence features.

    This covers DINOv2/DINOv3 and many ViT-like encoders without leaking
    model-specific code into the extraction script.
    """
...
class TimmBackbone(FrozenVisualBackbone):
    ...
    ``num_classes=0`` asks timm to expose the global feature vector instead of
    classifier logits. Model-native preprocessing is resolved from timm's data
    config and applied consistently for every batch.
    """
