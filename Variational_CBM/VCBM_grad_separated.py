...
def diagnose_gradient_separation(
    ...
    print("\n" + "=" * 60)
    print("GRADIENT SEPARATION DIAGNOSTIC")
    print("=" * 60)
    ...
    print(f"Weight std: mean={weight_std.mean():.4f}, min={weight_std.min():.4f}")

    if weight_std.mean() < 0.1:
        print("  ⚠️  WARNING: Weight std low - possible collapse!")
    else:
        print("  ✓ Weight std healthy")

    # Check alpha
    alpha = model.alpha_scheduler.get_alpha(model.current_epoch)
    results['alpha'] = alpha
    print(f"Current α: {alpha:.3f} (epoch {model.current_epoch})")
    ...
    if abs(rho) < 0.2:
        ...
        print(f"  ⚠️  Correlation too high (target < 0.1)")
    ...
    if len(all_errors) > 0:
        ...
        print(f"ρ(epistemic, error): {rho:.3f} (p={p:.2e})")

        if rho > 0.2:
            print("  ✓ Epistemic tracks errors!")
        else:
            print("  ⚠️  Epistemic not tracking errors well")
...
if __name__ == "__main__":
    ...
    print("\nExample output:")
    print(f"  Concept probs: {outputs['concept_probs'][0].numpy()}")
    print(f"  Epistemic (var): {outputs['epistemic_var'][0].mean().item():.4f}")
    print(f"  Epistemic (err): {outputs['epistemic_err'][0].mean().item():.4f}")
    print(f"  Epistemic (combined): {outputs['epistemic'][0].mean().item():.4f}")
    print(f"  Aleatoric: {outputs['aleatoric'][0].mean().item():.4f}")
    print(f"  α: {outputs['alpha']:.3f}")
