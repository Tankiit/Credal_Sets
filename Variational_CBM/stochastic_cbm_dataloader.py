"""
Stochastic CBM Multi-Dataset Dataloader v2
...
DATASET_INFO = {
    ...
    "hatexplain": {
        ...
        "use_legacy_loader": True,  # Flag to use direct dataset loading
    },
...
class StochasticCBMDataset(Dataset):
    """Universal dataset class for Stochastic CBM."""
    ...
    def _load_data(self, split: str):
        ...
        try:
            # Special handling for legacy datasets like HateXplain
            if info.get("use_legacy_loader", False):
                from datasets import load_dataset as hf_load
                print(f"  Using legacy loader for {self.dataset_name}")
                ds = hf_load(info["hf_path"], split=split_name)
            elif hf_name:
            ...
            print(f"  Trying to load with trust_remote_code=False...")
            try:
                if info.get("use_legacy_loader", False):
                    from datasets import load_dataset as hf_load
                    ds = hf_load(info["hf_path"], split=split_name)
                elif hf_name:
                    ds = hf_load_dataset(info["hf_path"], name=hf_name, split=split_name, trust_remote_code=False)
                else:
                    ds = hf_load_dataset(info["hf_path"], split=split_name, trust_remote_code=False)
    ...
    def _load_hatexplain(self, ds):
        """
        Load HateXplain dataset with enhanced concept encoding.
        ...
        Enhanced Concept encoding (ternary):
          - Concept 1 (has_target):
        ...
        for sample in ds:
            ...
            # LABEL ENCODING
            # =====================================================================
            # HateXplain labels: 0=hatespeech, 1=normal, 2=offensive
            ...
            # =====================================================================
            # CONCEPT 1: HAS_TARGET (Enhanced)
            # =====================================================================
            # Check how many annotators identified targets
            annotators_with_targets = sum(
                1 for t in targets
                if isinstance(t, list) and len(t) > 0
            )

            target_ratio = annotators_with_targets / total_annotators

            if target_ratio == 0:
                # No annotator found a target
                target_concept = 0  # No target
            elif target_ratio >= 0.67:
                # At least 2/3 found a target (strong agreement)
            ...
            # =====================================================================
            # CONCEPT 2: IS_OFFENSIVE (Enhanced)
            # =====================================================================
            # Count offensive annotations (hatespeech=0 or offensive=2)
            offensive_count = sum(1 for l in labels if l in [0, 2])
            normal_count = sum(1 for l in labels if l == 1)

            offensive_ratio = offensive_count / total_annotators

            if offensive_ratio >= 0.67:
                # At least 2/3 say offensive/hate
                offensive_concept = 2  # Offensive
            elif normal_count / total_annotators >= 0.67:
                # At least 2/3 say normal
                offensive_concept = 0  # Normal
            else:
                # Mixed opinions (disagreement)
                offensive_concept = 1  # Unclear/Disagree

            # =====================================================================
            # OPTIONAL: Add severity and consensus as additional signals
            # =====================================================================
            # You could track these for analysis even if not used as concepts:
            severity_score = sum(1 for l in labels if l == 0) / total_annotators  # Hatespeech ratio
            consensus_score = agreement_ratio  # How much annotators agree

            # =====================================================================
            # CREATE CONCEPT ARRAY
            # =====================================================================
            concepts = np.array([target_concept, offensive_concept], dtype=np.int64)
            is_unknown = (concepts == 1).astype(np.float32)

            # =====================================================================
            # OPTIONAL: Compute annotator entropy for aleatoric validation
            # =====================================================================
            # This is useful for validating aleatoric uncertainty
            if total_annotators > 1:
                label_probs = np.array([
                    label_counts.get(i, 0) / total_annotators
                    for i in range(3)
                ])
                label_probs = label_probs[label_probs > 0]  # Remove zeros
                annotator_entropy = -np.sum(label_probs * np.log(label_probs + 1e-10))
                # Normalize to [0, 1]
                annotator_entropy = annotator_entropy / np.log(3)
            else:
            ...
            self.examples.append({
                ...
                "_raw_labels": labels,  # For debugging
            })
    ...
    def __getitem__(self, idx):
        ...
        # Add optional metadata for analysis (if available)
        for key in ['_severity_score', '_consensus_score', '_annotator_entropy']:
            if key in ex:
                item[key] = torch.tensor(ex[key], dtype=torch.float)
...
def load_dataset_splits(
    ...
    # Load splits
    train_ds = StochasticCBMDataset(dataset_name, "train", tokenizer, config)
    val_ds = StochasticCBMDataset(dataset_name, "val", tokenizer, config)
    test_ds = StochasticCBMDataset(dataset_name, "test", tokenizer, config)
