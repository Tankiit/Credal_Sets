"""
MAQA/AmbigQA credal model components.
"""
...
def maqa_collate_fn(batch):
    if batch and "amb" in batch[0]:
        amb_batch = [item["amb"] for item in batch]
        clear_batch = [item["clear"] for item in batch]
        return {"amb": collate_single(amb_batch), "clear": collate_single(clear_batch)}
...
class CredalMAQA(nn.Module):
    ...
    def forward(
        ...
        mu = self.mu_head(projected)
        sigma_epi = self.sigma_epi_head(projected).squeeze(-1)
        sigma_ale = self.sigma_ale_head(projected).squeeze(-1)
        params = CredalQAParameters(mu, sigma_epi, sigma_ale)
...
class MAQACredalLoss(nn.Module):
    ...
    def forward(
        ...
        for i in range(batch_size):
            num_answers = int((p_star[i] > 0).sum().item())
            if num_answers > 0:
                ...
                kl_losses.append(kl)
        loss_kl = torch.stack(kl_losses).mean() if kl_losses else torch.tensor(0.0, device=p_star.device)
        losses["loss_kl"] = loss_kl

        loss_reg = F.relu(torch.tensor(0.1, device=params.sigma_epi.device) - torch.abs(params.sigma_epi - params.sigma_ale)).mean()
        ...
        if params_clear is not None:
            ...
            loss_cont = torch.tensor(0.0, device=params.sigma_ale.device)
...
class MAQACredalTrainer:
    ...
    def train_epoch(self) -> Dict:
        self.model.train()
        total_loss = total_answer_loss = total_reg_loss = total_cal_loss = total_cont_loss = 0.0
        total_correct = total = 0
        all_sigma_epi, all_sigma_ale, all_entropy_gt = [], [], []
        ...
        for batch in tqdm(self.train_loader, desc="MAQA train"):
            ...
            total_answer_loss += float(losses["loss_kl"].item())
            total_reg_loss += float(losses["loss_reg"].item())
            total_cal_loss += float(losses["loss_cal"].item())
            total_cont_loss += float(losses["loss_cont"].item())
            ...
            all_entropy_gt.append(batch_metrics["entropy"].detach().cpu())
        ...
        return {
            ...
            "answer_loss": total_answer_loss / max(1, len(self.train_loader)),
            "kl_reg_loss": total_reg_loss / max(1, len(self.train_loader)),
            "calibration_loss": total_cal_loss / max(1, len(self.train_loader)),
            "contrastive_loss": total_cont_loss / max(1, len(self.train_loader)),
            ...
            "mean_entropy_gt": float(all_entropy_gt.mean().item()),
        }
    ...
    def _step(self, batch):
        if "amb" in batch:
            ...
            return loss, losses, params_amb, amb
        ...
        return loss, losses, params, batch
    ...
    def evaluate(self, loader) -> Dict:
        ...
        metrics = {
            ...
            "task_accuracy": total_correct / max(1, total),
            "answer_loss": total_answer_loss / max(1, len(loader)),
            "kl_reg_loss": total_reg_loss / max(1, len(loader)),
            "calibration_loss": total_cal_loss / max(1, len(loader)),
            "contrastive_loss": total_cont_loss / max(1, len(loader)),
            "mean_sigma_epi": float(all_sigma_epi.mean().item()),
            "mean_sigma_ale": float(all_sigma_ale.mean().item()),
            "mean_entropy_gt": float(all_entropy_gt.mean().item()),
        }
        if len(all_sigma_epi) > 10:
            ...
            metrics["rho_au_entropy"] = float(stats.pearsonr(all_sigma_ale.numpy(), all_entropy_gt.numpy())[0])
        return metrics
    ...
    def fit(self, num_epochs: int, save_dir: Path, eval_every: int = 1):
        ...
        for epoch in range(1, num_epochs + 1):
            ...
            if score > best:
                best = score
                torch.save({"model_state_dict": self.model.state_dict()}, save_dir / "best_model.pt")
        (save_dir / "training_history.json").write_text(json.dumps({"history": self.history}, indent=2, default=float))
    ...
    def dump_eval_arrays(self, loader, out_dir: Path):
        ...
        np.savez(
            ...
            entropy_gt=torch.cat(all_entropy, dim=0).numpy(),
        )
